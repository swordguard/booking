        ...
        <fieldset id="group">
          {Array(availableTimeSlots)
            ...
            .map((v, i) => {
              ...
              return (
                <div key={i} className={disabled ? "disabled-radio" : ""}>
                  {Number(timeSlot) === i ? (
                    <input
                      type="radio"
                      key={i}
...
const MyDateAndTimePicker = ({
  ...
  const [startDate, setStartDate] = useState(
    bookedSession && bookedSession.sessionName === sessionName
      ? new Date(Number(bookedSession.bookedDate))
      : new Date()
  );
  ...
  useEffect(() => {
    ...
    if (occupiedSlots) {
      const filteredSlotsDates = occupiedSlots.filter((slot) => {
        ...
        return (
          `${occupiedYear}${occupiedMonth}${occupiedDate}` ===
          `${selectedYear}${selectedMonth}${selectedDate}`
        );
      });
      if (filteredSlotsDates.length > 0) {
        const updatedDisabledSlots = filteredSlotsDates.map(
          (slot) => Object.values(slot)[0]
        );
  ...
  return (
    <>
      ...
      />
      <TimeRangeSelection
        onTimeSelect={() => onTimeSelectWithDate(startDate)}
        timeSlot={timeSlot}
